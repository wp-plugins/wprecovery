...
class OCR_Backup {
	...
	public function get_archive_filename() {
		...
		{
			$archive_file_name = str_replace('.sql','.zip',$this->get_database_dump_filename());
			$this->set_archive_filename($archive_file_name);
		}
	...
	public function get_database_dump_filename() {
		...
		if ( empty( $this->database_dump_filename ) )
			...
			$this->set_database_dump_filename( implode( '-', array( sanitize_title( str_ireplace( array( 'http://', 'https://', 'www' ), '', home_url() ) ), 'backup', date( 'Y-m-d-H-i-s', current_time( 'timestamp' ) ) ) ) . '.sql');
    ...
    public function get_path() {
		...
		if ( empty( $this->path ) )
			$this->set_path(WP_CONTENT_DIR.'/wprecovery_backup/' );
		//echo $this->path;die;
        return $this->path;
	...
	public function backup() {
		...
		$this->do_action( 'ocrb_backup_started' );
		...
		$this->do_action( 'ocrb_backup_complete' );
		$full_path = $this->get_archive_filepath();
		do_action( '_ocrb_backup_complete', $this->get_archive_filepath() );
		return $full_path;
	}
	...
	public function dump_database() {
		...
		$this->do_action( 'ocrb_mysqldump_finished' );
	...
	public function mysqldump() {
		...
		$this->do_action( 'ocrb_mysqldump_started' );
		...
		if ( ! defined( 'OCRB_MYSQLDUMP_SINGLE_TRANSACTION' ) || OCRB_MYSQLDUMP_SINGLE_TRANSACTION !== false )
	...
	public function archive() {
		...
		$this->do_action( 'ocrb_archive_finished' );
	...
	public function zip() {
		...
		$this->do_action( 'ocrb_archive_started' );
	...
	public function pcl_zip() {
		...
		global $_ocrb_exclude_string;

		$_ocrb_exclude_string = $this->exclude_string( 'regex' );
		...
		// Zip up everything
		if ( $this->get_type() !== 'database' )
			if ( ! $archive->add( $this->get_root(), PCLZIP_OPT_REMOVE_PATH, $this->get_root(), PCLZIP_CB_PRE_ADD, 'ocrb_pclzip_callback' ) )
		...
		unset( $GLOBALS['_ocrb_exclude_string'] );
	...
	public function verify_mysqldump() {
		...
		$this->do_action( 'ocrb_mysqldump_verify_started' );
	...
	public function verify_archive() {
		...
		$this->do_action( 'ocrb_archive_verify_started' );
 ...
 * @return bool
 */
function ocrb_pclzip_callback( $event, &$file ) {
    ...
    elseif ( $_ocrb_exclude_string && preg_match( '(' . $_ocrb_exclude_string . ')', $file['stored_filename'] ) )
    	return false;
