...
class OCRB_Scheduled_Backup extends OCR_Backup {
	...
	public function __construct( $id ) {
		...
		// Load the options
		$this->options = array_filter( (array) get_option( 'ocrb_schedule_' . $this->get_id() ) );

		// Some properties can be overridden with defines
		if ( defined( 'OCRB_ROOT' ) && OCRB_ROOT )
			$this->set_root( OCRB_ROOT );

		if ( defined( 'OCRB_PATH' ) && OCRB_PATH )
			$this->set_path( OCRB_PATH );

		if ( defined( 'OCRB_EXCLUDE' ) && OCRB_EXCLUDE )
			parent::set_excludes( OCRB_EXCLUDE, true );

		parent::set_excludes($this->default_excludes(), true );

		if ( defined( 'OCRB_MYSQLDUMP_PATH' ) )
			$this->set_mysqldump_command_path( OCRB_MYSQLDUMP_PATH );

		if ( defined( 'OCRB_ZIP_PATH' ) )
			$this->set_zip_command_path( OCRB_ZIP_PATH );

		if ( defined( 'OCRB_ZIP_PATH' ) && OCRB_ZIP_PATH === 'PclZip' && $this->skip_zip_archive = true )
			$this->set_zip_command_path( false );

		// Set the path - TODO remove external function dependancy
		$this->set_path( ocrb_path() );

		$ocrb_schedules = $this->get_cron_schedules();
		...
		if ( ( ! $this->get_next_occurrence() && in_array( $this->get_reoccurrence(), array_keys(  $ocrb_schedules ) ) ) || ( date( get_option( 'time_format' ), strtotime( OCRB_SCHEDULE_TIME ) - ( get_option( 'gmt_offset' ) * 3600 ) ) !== date( get_option( 'time_format' ), $this->get_next_occurrence() ) ) )
			$this->schedule();
	...
	public function set_excludes( $excludes, $append = false ) {
		...
		// Use the validation from OCR_Backup::set_excludes
	...
	public function get_filesize( $cached = true ) {
		...
		if ( $cached ) {
			...
			$filesize = get_transient( 'ocrb_schedule_' . $this->get_id() . '_filesize' );
			...
			if ( $filesize === 'calculating' ) {
				...
				while ( 'calculating' === ( $filesize = $wpdb->get_var( "SELECT option_value FROM $wpdb->options WHERE option_name = '_transient_ocrb_schedule_" . $this->get_id() . "_filesize'" ) ) ) {
		...
		set_transient( 'ocrb_schedule_' . $this->get_id() . '_filesize', 'calculating', time() + HOUR_IN_SECONDS );
    	...
    	// Don't include files if database only
   		if ( $this->get_type() != 'database' ) {
			...
			foreach ( $this->get_files() as $file ) {
			    ...
			    // Excludes
			    if ( $excludes && preg_match( '(' . $excludes . ')', str_ireplace( trailingslashit( $this->get_root() ), '', OCR_Backup::conform_dir( $file->getPathname() ) ) ) )
			        continue;
		...
		// Cache for a day
		set_transient( 'ocrb_schedule_' . $this->get_id() . '_filesize', $filesize, time() + DAY_IN_SECONDS );
	...
	public function is_filesize_cached() {
		...
		$size = get_transient( 'ocrb_schedule_' . $this->get_id() . '_filesize' );
	...
	public function clear_filesize_cache() {
		delete_transient( 'ocrb_schedule_' . $this->get_id() . '_filesize' );
	}
	...
	public function get_schedule_start_time() {
		...
		if ( empty( $this->schedule_start_time ) ) {
			...
			if ( defined( 'OCRB_SCHEDULE_TIME' ) && OCRB_SCHEDULE_TIME )
				$date = strtotime( OCRB_SCHEDULE_TIME );
	...
	public function set_reoccurrence( $reoccurrence ) {
		...
		// Check it's valid
		if ( ! is_string( $reoccurrence ) || ! trim( $reoccurrence ) || ( ! in_array( $reoccurrence, array_keys( $ocrb_schedules ) ) ) && $reoccurrence !== 'manually' )
	...
	public function get_interval() {
		...
		return $ocrb_schedules[$this->get_reoccurrence()]['interval'];
	...
	public function get_cron_schedules(){
		...
		// remove any schedule whose key is not prefixed with 'ocrb_'
		foreach ( $schedules as $key => $arr ) {
			if( ! preg_match("/^ocrb_/", $key ) )
				unset( $schedules[$key] );
		}
	...
	public function get_next_occurrence( $gmt = true ) {
		...
		$time = wp_next_scheduled( 'ocrb_schedule_hook', array( 'id' => $this->get_id() ) );
	...
	public function schedule() {
		...
		wp_schedule_event( $this->get_schedule_start_time(), $this->get_reoccurrence(), 'ocrb_schedule_hook', array( 'id' => $this->get_id() ) );
	...
	public function unschedule() {
		wp_clear_scheduled_hook( 'ocrb_schedule_hook', array( 'id' => $this->get_id() ) );
	}
	...
	public function run() {
		...
		$this->set_status( __( 'Starting Backup', 'ocrb' ) );
	...
	protected function do_action( $action ) {
		...
		switch ( $action ) :
	    	...
	    	case 'ocrb_mysqldump_started' :

	    		$this->set_status( sprintf( __( 'Dumping Database %s', 'ocrb' ), '(<code>' . $this->get_mysqldump_method() . '</code>)' ) );

	    	break;

	    	case 'ocrb_mysqldump_verify_started' :

	    		$this->set_status( sprintf( __( 'Verifying Database Dump %s', 'ocrb' ), '(<code>' . $this->get_mysqldump_method() . '</code>)' ) );

	    	break;

			case 'ocrb_archive_started' :

	    		$this->set_status( sprintf( __( 'Creating zip archive %s', 'ocrb' ), '(<code>' . $this->get_archive_method() . '</code>)' ) );

	    	break;

	    	case 'ocrb_archive_verify_started' :

	    		$this->set_status( sprintf( __( 'Verifying Zip Archive %s', 'ocrb' ), '(<code>' . $this->get_archive_method() . '</code>)' ) );

	    	break;

	    	case 'ocrb_backup_complete' :

	    		$this->set_status( __( 'Finishing Backup', 'ocrb' ) );
	    ...
	    foreach ( OCRB_Services::get_services( $this ) as $service )
	...
	public function save() {
		...
		if ( $this->options !== get_option( 'ocrb_schedule_' . $this->get_id() ) )
			update_option( 'ocrb_schedule_' . $this->get_id(), $this->options );
	...
	public function cancel() {
		...
		delete_option( 'ocrb_schedule_' . $this->get_id() );
	...
	public function default_excludes() {
		...
		return apply_filters( 'ocrb_default_excludes', $excluded );
	}
